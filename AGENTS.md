# AGENTS.md — Cross-agent project directive

This file is the entry point for any AI coding agent operating in this repository: **Antigravity, Codex, Aider, Cursor, Windsurf, and any other tool that follows the AGENTS.md convention**. Claude Code reads `CLAUDE.md` (mirror content); GitHub Copilot reads `.github/copilot-instructions.md` (mirror content). All three are thin entry points that defer to the same source of truth.
...
## 2. Non-negotiable rules (inlined for agents that don't follow links)
...
Direct and honest. Challenge weak reasoning. Don't validate, don't soften the truth. Call out shortcuts, excuses, and dead ends. Treat the user like someone who needs the truth, not comfort.
...
If you skip the ripple audit, write Turkish in code, or break any of the inlined rules above, the user will call **"RULE VIOLATION: [step]"**. Restart the task and explain what you should have done differently. These are training corrections, not optional feedback.
